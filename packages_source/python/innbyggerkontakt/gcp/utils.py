...
class CloudStorage:
    ...
    def get_file_from_bucket(self, name, as_json=True) -> Union[dict, str]:
        """Load and return json object from bucket."""
...
def parse_firestore(data: dict) -> dict:  # noqa: CCR001
    """Parse cloud_functions firestore trigger data."""
    my_dict = {}
    basic_types = ['stringValue', 'integerValue', 'booleanValue']
    types = ['mapValue', 'fields', 'arrayValue']
    for key, value in data.items():
        if key in basic_types:
            return value
        elif key == 'timestampValue':  # TODO ASN: this could be it's own method
            if 'Z' in value:
                value = str(value).replace('Z', '+00:00')
            tz = pytz.timezone('europe/oslo')
            return datetime.fromisoformat(value).replace(tzinfo=pytz.utc).astimezone(tz)
        elif key in types:
            return parse_firestore(value)
        elif key == 'values':
            return [parse_firestore(item) for item in value]
        my_dict.update({key: parse_firestore(value)})
    return my_dict
