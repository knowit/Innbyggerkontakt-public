...
def get_user(token: str) -> User:
    ...
    org_user = db.collection('organization').document(org_id).collection('users').document(uid).get().to_dict()
    return User(user_id=uid, organization_id=data['orgId'], organization_role=org_user['rolle'])


def authenticate(get_user_as_kwarg=None):
